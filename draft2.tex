...
\title{Topological Sigma Models from Generalized Para-Complex Geometry}
...
\paragraph{Type decomposition} \david{simplify this section} The splitting of the tangent bundle $T\PS=L\oplus \Lt$ in the real case and $T\PS^\Cc$ in the para-complex case gives rise to a decomposition of tensors analogous to the $(p,q)$-decomposition in complex geometry. For differential forms, we denote the real and para-complex decompositions as
...
\brian{motivate generalized geometry through courant algebroids}

\brian{get rid of repetitions and references to dg / NQ manifolds.}
...
For now, we record a relationship between generalized paracomplex structures and Courant algebroids. 

\subsubsection{Background on Courant algebroids}

Recall, a Courant algebroid on a manifold $M$ is a vector bundle $E$, whose sheaf of sections we denote $\cE$, together with a nondegenerate symmetric bilinear pairing $\langle -, -\rangle : \cE \otimes \cE \to C^\infty(M)$ , an anchor map $a : E \to T M$, and a bilinear operator
\[
[-,-] : \cE \times \cE \to \cE
\]
...
\paragraph{The $2$-shifted symplectic space of an exact Courant Algebroid}
...
\paragraph{The Poisson Courant algebroid}
...
The Courant algebroid on $\TT$ is exact
In fact, all possible Courant algebroid structures on $\TT$ are parametrized by a closed three-form $H\in \Omega^3_{cl}$, which enters the definition of the bracket \eqref{eq:dorfman}, changing it to a {\it twisted} Dorfman bracket
\begin{align*}
...
\begin{remark}
\brian{maybe this should go later?}
...
\subsection{$(2,2)$ para-supersymmetry}
\david{Import paragraph on (2,2)}
...
\david{Move to mirror symmetry speculation section}
...
\brian{maybe leave this as a remark that we are aware of this...but we don't need it.}
...
\section{AKSZ and generailzed para-geometry}

In this section we will show how to associate $2D$ topological theories to a generalized (para-)complex structure.
Later on, in Section \ref{sec:toptwist}, we relate these theories to topological twists of $2D$ $(2,2)$ para-SUSY sigma models introduced in Section \ref{sec: parasusy}. 
%and in \cite{Kapustin:2004gv}. 
...
For instance, $\cE(M)$ will be a mapping space of the form ${\rm Map}(M, X)$ where $X$ is shifted symplectic. 
