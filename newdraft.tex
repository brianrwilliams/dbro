...
\maketitle
\begin{abstract}
We study a class of $2d$ topological sigma models that are formulated on generalized para-complex target manifolds via the AKSZ framework. When the target is additionally generalized para-K\"ahler, these topological theories can be realized as topological twists of $(2,2)$ para-supersymmetric sigma models of Abou-Zeid and Hull. We compare our results to the case of generalized complex and generalized K\"ahler geometry, where similar results have been described in the context of the usual $(2,2)$ SUSY by Kapustin and Li. We present several examples and show that our construction recovers the A-model and the Poisson sigma model as special cases. Moreover, we introduce new examples of topological theories inherently tied to the para-complex geometry.
...
\paragraph{Main results} In this paper, we combine the results of \cite{HullTwistedSUSY} and \cite{Hu:2019zro} to study the $2D$ topological sigma models with a generalized para-complex target manifold $(\Mb,\KK_+)$. This geometry leads to a $2D$ topological theory via the AKSZ formalism as follows. The Courant algebroid $(\TT)\Mb$ defines a $3D$ topological theory with a $2$-shifted symplectic NQ target space $({\cal E},\Omega)$ \cite{Roytenberg:2002nu}. Because the $\pm 1$-eigenbundles of $\KK_+$ are Dirac structures in $(\TT)\Mb$, they each define a Lagrangian in $({\cal E},\Omega)$, which are $1$-shifted symplectic NQ manifolds and as such define a {\bf pair} of $2D$ topological boundary theories.

A new feature of the AKSZ theories formulated on the generalized para-complex target is that the above-described pair of $2D$ topological theories can be combined into a {\bf single} $2D$ {\bf paracomplexified} theory, i.e. the fields become $C^\infty(\Cc)$-modules (as opposed to $C^\infty(\RR)$- or $C^\infty(\mathbb{C})$-modules in the case of real and complex theories) \david{maybe there's a better defitition of para-complexified theory? Maybe we wanna call it para-complex instead of para-complexified?}, where $\Cc=\RR \otimes (1,k)$ is the algebra of {\bf para-complex numbers} and $k^2=1$ is the para-complex unit. Conversely, we find an analogue of the construction of the $2D$ theory in the para-complexified case. Let $(\Mb,\KK_+)$ be the same generalized para-complex target. Then $(\TT)_\Cc \Mb=((\TT)\otimes_\RR \Cc) \Mb$ defines a para-complexified $3D$ AKSZ theory and the $+k$-eigenbundle of $\KK_+$ in $(\TT)_\Cc \Mb$ defines a $2D$ topological boundary condition. This theory then corresponds to the two real theories we described in the previous paragraph.

Additionally, we show that when the GpC manifold $(\Mb,\KK_+)$ also admits a generalized para-K\"ahler structure $(\Mb,\KK_+,\KK_-)$, the topological theories for $(\Mb,\KK_+)$ described above can be realized as topological twists of the $(2,2)$ para-SUSY theory attached to $(\Mb,\KK_\pm)$. A similar statement holds in the para-complexified case, in which case we consider a para-complexified $(2,2)$ para-SUSY algebra and the resulting theories are para-complexified as well.

We also present several explicit examples of our construction. The well-known examples include the Poisson sigma model of a real Poisson manifold $(\Mb,\Pi)$ that arises by considering the $-1$-eigenbundle of the GpC structure
\begin{align*}
\KK_\Pi=
\begin{pmatrix}
\id & 2\Pi \\
...
\end{pmatrix}.
\end{align*}
The A-model of a symplectic manifold $(\Mb,\omega)$ is recovered by considering either of the real eigenbundles of the GpC structure
\begin{align*}
\KK_\omega=
\begin{pmatrix}
0 & \omega^{-1} \\
...
\end{pmatrix},
\end{align*}
and the BF theory \david{(?)} is simply given by the trivial GpC structure
\begin{align*}
\KK_0=
\begin{pmatrix}
\id & 0 \\
...
The new examples that we describe are all tied to the para-complex geometry of the target. They are the para-complex version of the B-model of a para-complex manifold $(\Mb,K)$ that is given by the para-complex $+k$-eigenbundle of the GpC structure
\begin{align*}
\KK_K=
\begin{pmatrix}
K & 0 \\
...
\end{pmatrix},
\end{align*}
and the two real theories defined by the real eigenbundles of $\KK_K$, which can be understood as the BF theories of the foliation manifolds corresponding to the eigenbundles of $K$, which are integrable subbundles of $T\Mb$.
...
Notice, however, that for example the usual Poisson sigma model cannot be obtained in the realm of GC geometry because in general, a real Poisson manifold does not admit any GC structures.
...
In particular, $\phi$ maps between two copies of $\TT$ and $\TT_\Cc$
...
\end{enumerate}
%\begin{itemize}
%\item $E=TM\rightarrow M$, $a=\id_{TM}$, $\brac_E=\brac_M$, and
%\item $E=T\Mt\rightarrow \Mt$, $a=\id_{T\Mt}$, $\brac_E=\brac_\Mt$,
%\end{itemize}
...
%  and that we will use in our identification of the boundary $2D$ TFT's. 
