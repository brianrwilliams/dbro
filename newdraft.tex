...
The Lie algebroid $L_{\beta}$ is equivalent to the Lie algebroid with underlying bundle $T^*$ and anchor given by the Poisson bivector $\beta : T^* \to T$. 
\end{Ex}
...
\btd{Is there a classification of a most general GpC that can be written in the form
\[
\KK = 
\begin{pmatrix}
K & P \\
...
\end{pmatrix}
\]
where $K \in {\rm End}(T)$, $P \in \wedge^2 T$ and $\omega \in \wedge^2 T^*$ satisfying some integrability conditions?
}
